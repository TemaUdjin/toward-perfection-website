...
export function LessonPlayer({ lesson, index }: { lesson: Lesson; index: number }) {
    ...
    <div className={`rounded-2xl border transition-colors overflow-hidden ${
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-[var(--muted-foreground)] mt-0.5">{lesson.duration}</p>
        </div>
        <svg
      ...
      {open && (
        <div className="border-t border-[var(--border)]">
          ...
          <div className="px-6 pb-4">
            <p className="text-sm text-[var(--muted-foreground)] leading-relaxed">{lesson.description}</p>
          </div>
